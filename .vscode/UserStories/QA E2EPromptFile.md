...
1. Explore the application URL mentioned in the user story
2. Create a comprehensive test plan covering all acceptance criteria
3. Document each test scenario with:
   - Test case name
   - Pre-conditions
...
### Expected Output
- Comprehensive test plan saved to specs/
- All acceptance criteria covered
- Test scenarios with steps defined
...
### Expected Output
- Playwright JavaScript test scripts
- Scripts saved in tests/saucedemo-checkout/
...
- Descriptive commit message attached
- Changes pushed to remote repository

---

## ⚡ Single Combined Prompt (For Demo/Video)
...
Save it as: specs/saucedemo-checkout-test-plan.md

STEP 3 - EXPLORATORY TESTING:
Read the test plan from specs/saucedemo-checkout-test-plan.md and use Playwright browser tools
to manually execute each test scenario. Document findings with screenshots and note any issues discovered.

STEP 4 - GENERATE AUTOMATION SCRIPTS:
Review both the test plan (specs/saucedemo-checkout-test-plan.md) and exploratory testing results
from Step 3. Use the playwright-test-generator agent to create JavaScript automation scripts using
the element selectors and insights discovered during manual testing.
Save scripts in tests/saucedemo-checkout/.

STEP 5 - EXECUTE AND HEAL TESTS:
Run all automation scripts from tests/saucedemo-checkout/. Use the playwright-test-healer agent
to identify and auto-heal any failing tests. Re-run tests until all are stable and passing.
...
(execution and healing). Include PASS/FAIL status, healing summary, defects log, and test coverage analysis.
